<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$query_config   = mysqli_query($obj_conexion,"SELECT * FROM configuracion");
		...
		$query = mysqli_query($obj_conexion,"SELECT f.nofactura, DATE_FORMAT(f.fecha, '%d/%m/%Y') as fecha, DATE_FORMAT(f.fecha,'%H:%i:%s') as  hora, f.codcliente, f.estatus,
		...
		if($result > 0){
			...
			$query_productos = mysqli_query($obj_conexion,"SELECT p.descripcion,dt.cantidad,dt.precio_venta,(dt.cantidad * dt.precio_venta) as precio_total
														FROM factura f
